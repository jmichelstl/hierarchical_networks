...
/*
 * I, Jonathan Michel, have modified the names of the types Point and Edge to
 * be VorPoint and VorEdge, so they do not conflict with structs of type Point
 * and Edge I have defined elsewhere. This does not constitute a substantive
 * contribution, and it's possible that there is a more elegant solution, but
...
bool VoronoiDiagramGenerator::ELinitialize()
{
	...
	ELleftend = HEcreate( (struct VorEdge *)NULL, 0);
	ELrightend = HEcreate( (struct VorEdge *)NULL, 0);
...
struct Halfedge* VoronoiDiagramGenerator::HEcreate(struct VorEdge *e,int pm)
{
...
struct Halfedge * VoronoiDiagramGenerator::ELgethash(int b)
{
	...
	he = ELhash[b]; 
	if (he == (struct Halfedge *) NULL || he->ELedge != (struct VorEdge *) DELETED ) 
		return (he);
...
void VoronoiDiagramGenerator::ELdelete(struct Halfedge *he)
{
	...
	he -> ELedge = (struct VorEdge *)DELETED;
}
...
{
	if(he -> ELedge == (struct VorEdge *)NULL) 
		return(bottomsite);
	return( he -> ELpm == le ? 
...
{
	if(he -> ELedge == (struct VorEdge *)NULL) //if this halfedge has no edge, return the bottom site (whatever that is)
		return(bottomsite);
...
struct VorEdge * VoronoiDiagramGenerator::bisect(struct Site *s1,struct	Site *s2)
{
	double dx,dy,adx,ady;
	struct VorEdge *newedge;	

	newedge = (struct VorEdge *) getfree(&efl);
...
struct Site * VoronoiDiagramGenerator::intersect(struct Halfedge *el1, struct Halfedge *el2, struct VorPoint *p)
{
	...
	e1 = el1 -> ELedge;
	e2 = el2 -> ELedge;
	if(e1 == (struct VorEdge*)NULL || e2 == (struct VorEdge*)NULL) 
...
void VoronoiDiagramGenerator::endpoint(struct VorEdge *e,int lr,struct Site * s)
{
	e -> ep[lr] = s;
	ref(s);
...
void VoronoiDiagramGenerator::out_bisector(struct VorEdge *e)
{
...
void VoronoiDiagramGenerator::out_ep(struct VorEdge *e)
{
...
void VoronoiDiagramGenerator::clip_line(struct VorEdge *e)
{
	struct Site *s1, *s2;
